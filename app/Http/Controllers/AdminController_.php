...
class AdminController extends Controller
{
    ...
    public function epinf() {return view('back.pages.fatura.epinf');}
    public function goldf() {return view('back.pages.fatura.goldf');}
    public function pazarf(){return view('back.pages.fatura.pazarf');}
    public function siparisler() {return view('back.pages.siparisler.index');}
    ...
    public function odemeler_onayla($durum, $id)
    {
        ...
        if ($durum == 1) {
            $user->update([
                'bakiye' => $user->first()->bakiye + $users->amount,
            ]);
    ...
    public function yorumlar_onayla($durum, $id)
    {
        ...
        $ilanPazar = DB::table('pazar_yeri_ilanlar')->where('id', $ilan->ilan)->first();
        $pazar = DB::table('games_titles')->where('id', $ilanPazar->pazar)->first();
        $link = route('item_ic_detay', [$pazar->link, $ilanPazar->sunucu, Str::slug($ilanPazar->title) . '-' . $ilanPazar->id]);
        $alici = DB::table('users')->where('id', $ilanPazar->user)->first();
        if ($durum == 1) {
            ...
            setBildirim($ilanPazar->user, '2', 'İlanınızda Yeni Yorum Var', 'İlanınız için yeni bir yorum var. Görüntülemek için tıklayın.', $link);
            $smsText = "Sayın " . substr($alici->name, 0, 2) . "**** ******, " . $ilanPazar->id . " ID'li ilanınıza yorum yapılmıştır. Yanıtlamak için lütfen üye girişi yapınız.";
    ...
    public function ilanlar_yonetim_onay_ozel($id, $durum)
    {
        if ($durum == 1) {
            ...
            DB::table('users')->where('id', $alanSatis->satin_alan)->update([
                'bakiye' => $alici->bakiye + $alanSatis->price,
            ]);
    ...
    public function ilanlar_yonetim_buy_onay($id, $durum)
    {
        ...
        if($durum == 66) {
            ...
            if($alici->bakiye < ($ilan->price - $ilan->moment_komisyon)) {
             ...
             */
            DB::table('users')->where('id', $ilan->user)->update([
                'bakiye' => $alici->bakiye - ($ilan->price - $ilan->moment_komisyon),
            ]);
            LogCall(Auth::user()->id, '4', "Kullanıcı " . $id . " id'li buy ilan için alıcıdan ".$ilan->price - $ilan->moment_komisyon." TL para tahsil etti.");
            return back()->with("success", __('general.mesaj-9'));
        }
        DB::table('pazar_yeri_ilanlar_buy')->where('id', $id)->update([
            'status' => $durum,
    ...
    public function ilanlar_yonetim_buy_onay_red(Request $request)
    {
        ...
        DB::table('users')->where('id', $ilan->user)->update([
            'bakiye' => $alici->bakiye + $ilan->moment_komisyon,
        ]);
...
$stok_id=$userF->paket;
$once=$paket->stok;
if($userF->tur == "bize-sat") {$sonra=$once+$userF->adet;} else {$sonra=$once-$userF->adet;}
$islem=$userF->adet;
$admin=Auth::user()->id;
...
log_stok($admin,$stok_id,$once,'-'.$islem,$sonra,$det.$userF->price); 
    ...
    public function para_cek_yonetim_onayla($durum, $id)
    {
        ...
        if ($durum == 2) {
            $_GET['red']=isset($_GET['red'])?$_GET['red']:'';
            DB::table('para_cek')->where('id', $id)->update([
                'text' => $_GET['red'],
            ]);
            $para_cek = DB::table('para_cek')->where('id', $id)->first();
	...
	public function epin_rapor()		{return view('back.pages.rapor.epin-rapor.index');}	
	public function oyun_parasi_rapor()	{return view('back.pages.rapor.oyun-parasi-rapor.index');}
	public function ilan_rapor()		{return view('back.pages.rapor.ilan-rapor.index');}
	public function boss_rapor()		{return view('back.pages.rapor.boss-rapor.index');}
