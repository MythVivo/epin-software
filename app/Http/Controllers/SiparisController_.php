...
class SiparisController extends Controller
{
    ...
    public function odeme_yap(Request $request)
    {
        if ($request->tur == 1) { //online ödeme
            ...
            if ($tutar != '' or $tutar > 0) {
                ...
                if (!strstr($tutar, ",")) {
                    $total = number_format($tutar, 2, ",", ".");
                } else {
                    $total = $tutar;
                }
                $hash = hashHesapla($total, $lastId);
                $taksit = 1;
                ...
                $xml_post_string = '<?xml version="1.0" encoding="utf-8"?> <soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
                                        <soap:Body>
                                        ...
                                        <KK_No>' . $number . '</KK_No>
                                        <KK_SK_Ay>' . $expiration_month . '</KK_SK_Ay>
                                        <KK_SK_Yil>20' . $expiration_year . '</KK_SK_Yil>
                                        <KK_CVC>' . $cvv . '</KK_CVC>
                                        <KK_Sahibi_GSM>' . $telefon . '</KK_Sahibi_GSM>
                                        <Hata_URL>' . $merchant_fail_url . '</Hata_URL>
                                        <Basarili_URL>' . $merchant_ok_url . '</Basarili_URL>
                                        <Siparis_ID>' . $lastId . '</Siparis_ID>
                ...
                if ($result->Islem_ID == 0) {
                    ...
                    LogCall(Auth::user()->id, '3', "Kullanıcı ödeme talebi oluşturulamadı.");
                    ...
                    setBildirim(Auth::user()->id, '4', 'Ödeme Talebiniz Oluşturuldu', 'Ödeme talebiniz oluşturuldu, detaylar için tıklayın.', route('bakiye_ekle'));
                    echo "<script>window.location.href='" . $xml->Body->Pos_OdemeResponse->Pos_OdemeResult->UCD_URL . "';</script>";
                }
                die();
                */
                    ...
                    if ($kod->first()->expired_at > date('Y-m-d H:i:s')) {
                        ...
                        if ($paket->count() > 0) {
                            DB::table('odemeler')->insert([
                                ...
                                'channel' => 3,
                                'description' => $request->kod . " kodu kullanılarak yükleme yapıldı.",
                                'status' => 1,
                                'created_at' => date('YmdHis')
                            ]); //ödeme kaydı gerçekleştiriliyor
                            $kod->update(['isUsed' => '1']);
    ...
    public function odeme_notify(Request $request)
    {
      ...
      //  \Log::debug(serialize(['odeme_notify',request()->all(),request()->server()]));
        ...
        if(substr($post['merchant_oid'] ,0,4) == 'TEST') { echo 'OK'; exit; }
    ...
    function bakiye_cek_post(Request $request)
    {
        ...
        if ($bakiye_dusur) {
            ...
            DB::table('para_cek')->insert([
                ...
                'kesinti' => $papara==1?3.5:DB::table('settings')->first()->yayin_komisyon,
    ...
    public
    function bakiye_cek_odeme_kanali(Request $request)
    {
        DB::table('odeme_kanallari')->insert([
            'user' => Auth::user()->id,
            'title' => $request->title,
    ...
    public
    function bakiye_cek_odeme_kanali_duzenle(Request $request)
    {
