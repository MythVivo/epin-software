<?php
/*
 * Kullanıcının 3 veya daha fazla olumsuz alış ilanı varsa yeni satış yapamaz.
 */
$olumsuzKontrol = DB::table('pazar_yeri_ilanlar_buy')->where('status', '99')->where('user', Auth::user()->id)->where('olumsuz', '1')->count();
if (Auth::user()->alisIzin == 0) {
    header("Location: " . URL::to(route('alici_panelim')) . "?olumsuz=1");
    exit();
}
?>
@if(isset($_GET['pazar']) and isset($_GET['itemler']))
    ...
    @foreach(DB::table('games_titles_items_info')->where('game_title', $_GET['pazar'])->whereNull('deleted_at')->where('title', 'like', '%'.$_GET['searchTerm'].'%')->get() as $i)
        <?php
        $itemler[] = array('id' => $i->id, 'text' => $i->title);
        ?>
    @endforeach
                    ...
                    <div class="col-md-3">
                        @foreach($item_photo as $ip)
                            <img src="{{asset('public/front/games_items/' . $ip->image)}}"
                                 class="card-img-center bg-black"
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-4">
                                            ...
                                            <input onkeyup="komisyonHesaplaAlis(this)" id="2"
                                        ...
                                        <div class="col-md-8">
                                            <label for="5" class="form-label">Bloke Edilecek Tutar</label><br>
                                            <div class="input-group mb-3">
                                                <span class="input-group-text style-input" id="basic-addon1">10% tutar</span>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <div class="d-flex flex-row mb-2">
                                                ...
                                                <label for="7_24" class="form-check form-switch"
                                                    ...
                                                    <input name="7_24" id="7_24" type="checkbox"
                                                        role="switch">
                                                    <div class="switcher"><span><i></i></span></div>
                                                    <span class="form-label" style="min-width:70px">24 Saat <col></span>
                                                </label>
                                            </div>
                                        ...
                                        <div class="col-12 mt-3">
                                            ...
                                            <textarea id="4" class="form-control" name="text{{$item}}" placeholder="Açıklama" style="height: 140px"></textarea>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body bg-dark text-left">
                    <h5 style="color: var(--main-dark-link-color)!important; font-weight: 400;">
                        ...
                        3. ALICI ilan oluşturulabilmesi için almak istenilen ilan tutarının %10 ‘u kadar hesabında bakiye
                ...
                <div class="modal-footer bg-dark">
                    ...
                    <button type="button" data-bs-dismiss="modal" class="btn btn-success">Okudum, Anladım, Onaylıyorum</button>
                </div>
            </div>
        </div>
        <br>
        ...
        <div class="container">
            <div class="row">
                ...
                @if(session('error'))
                    ...
                    <form action="{{route('alici_panelim_post')}}" method="post" autocomplete="off"
                          enctype="multipart/form-data">
                        <div class="row g-3">
                            @csrf
                            <div class="col-md-12 select-element area">
                                <label for="pazar" class="form-label">Alış Yapacağınız Pazarı Seçin</label><br>
                                <select id="pazar" class="select2" name="pazar" style="width: 100%"
                                    ...
                                    @foreach(DB::table('games_titles')->where('type', '1')->whereNull('deleted_at')->get() as $u)
                                        @if(DB::table('games_titles_special')->where('games_titles', $u->id)->count() < 1)
                                            <option value="{{$u->id}}"
                                                    @if(isset($_GET['market']) and $_GET['market'] == $u->id) selected @endif>{{DB::table('games')->where('id', $u->game)->first()->title}}
                                                - {{$u->title}}</option>
                                        @endif
                                    @endforeach
                                </select>
            ...
            function itemGetir() {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        $(".area").html(result);
                        $(".area").append("<input id='pazar' type='hidden' name='pazar' value='" + pazar + "'>");
                        $(".select2").select2();
