...
@if(isset($_GET['sil']))
    ...
    if (DB::table('pazar_yeri_ilanlar_buy')->where('user',Auth::user()->id)->where('id', $_GET['sil'])->first()->status < 2) {
        ...
        if ($ilan->money_status == 0 and $ilan->deleted_at == NULL) {
            DB::table('users')->where('id', Auth::user()->id)->update(['bakiye' => $ilan->moment_komisyon + Auth::user()->bakiye]);
        }
    }
    DB::table('pazar_yeri_ilanlar_buy')->where('user',Auth::user()->id)->where('id', $_GET['sil'])->update([
                            ...
                            <ul class="nav nav-pills" id="pills-tab" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link @if(!session('panel') or (session('panel') and session('panel') != 3)) active @endif"
                                            id="pills-satista-tab" data-bs-toggle="pill"
                                            data-bs-target="#pills-satista" type="button" role="tab"
                                            aria-controls="pills-satista"
                                            @if(!session('panel') or (session('panel') and session('panel') != 3))
                                            aria-selected="true"
                                            @else
                                            aria-selected="false"
                                            @endif
                                ...
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" id="pills-sattiklarim-tab" data-bs-toggle="pill"
                                            ...
                                            aria-controls="pills-sattiklarim" aria-selected="false">Aldıklarım
                                    </button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link @if(session('panel') and session('panel') == 3)) active @endif"
                                            id="pills-alis-sattiklarim-tab" data-bs-toggle="pill"
                                            data-bs-target="#pills-alis-sattiklarim" type="button" role="tab"
                                            aria-controls="pills-alis-sattiklarim"
                                            @if(session('panel') and session('panel') == 3)) aria-selected="true"
                                            @else aria-selected="false" @endif>Sattıklarım
                                    </button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" type="button"
                                            onclick="location.href='{{route('yeni_satis_buy')}}'">
                                        Yeni Alış
                                    </button>
                                </li>
                            </ul>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <button data-bs-toggle="modal"
                                                                    ...
                                                                    type="button"
                                                                    class="btn btn-lg btn-outline-info waves-effect waves-light">
                                                                <i class="far fa-comments"></i>
                                                            </button>
                                                            @if($u->status < 2)
                                                                <button data-bs-toggle="modal"
                                                                        title="İlanı Düzenle"
                                                                        data-bs-target="#duzenle{{$u->id}}"
                                                                        onclick="selectCalistir({{$u->id}})"
                                                                        type="button"
                                                                ...
                                                                <button confirm-data='?sil={{$u->id}}' type="button"
                                                                        title="İlanı Sil"
                                                                        class="btn btn-lg btn-outline-danger waves-effect waves-light confirm-btn">
                                                                                        ...
                                                                                        <div class="col-6">
                                                                                            ...
                                                                                            <small class="text-danger">
                                                                                                * Hesabınızda bulunması
                                                                                                ...
                                                                                                işlem de
                                                                                                yeni komisyon <span
                                                                                                        class="text-info yeniKomisyon">{{$u->moment_komisyon}}</span>
                                                                                                TL - eski komisyon <span
                                                                                                        class="text-info eskiKomisyon">{{$u->moment_komisyon}}</span>
                                                                                                TL şeklinde
                                                                                                hesaplanmakta.
                                                                                                <br>
    ...
    <script>
        ...
        $(document).ready(function () {
            $(".girilenFiyat").keyup(function () {
                var komisyonOran = {{findUserKomisyonAlis(Auth::user()->id)}} / 100;
