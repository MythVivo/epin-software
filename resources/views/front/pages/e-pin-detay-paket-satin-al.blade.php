...
?>
@section('head')
    <meta name="description" content="{{ @$paket->title }} Satın Al">
    <meta name="keywords" content="{{ @$paket->title }} Satın Al">
@endsection
...
#---------------------------------------------------------------------------------------------------------------------------------
        ...
        $indirimli= $paket->price - ($al->epin * $paket->price / 100); // indirimli rakam
        $kam_fiy=findGamesPackagesPrice($paket->id); // kampanyalı rakam ne ?
        $satis_fiyati= $kam_fiy>$indirimli?$indirimli:$kam_fiy; // hangisi uygunsa onu yazıyoruz
        $oran=$al->epin;
    } else{ $satis_fiyati=findGamesPackagesPrice($paket->id); $oran=$paket->discount_amount;} // refid yoxa dewam
#---------------------------------------------------------------------------------------------------------------------------------

    $fiyat = $satis_fiyati * $adet;  //findGamesPackagesPrice($paket->id) * $adet;
    if ($paket->stockCode != null) {
        // eğer epin oyunu ise
        $ch = curl_init();
        $headers = ['Authorization: ' . getAuthName(), 'ApiName: ' . getApiName(), 'ApiKey: ' . getApiKey(), 'Content-Type: application/x-www-form-urlencoded'];
        curl_setopt($ch, CURLOPT_URL, env('EPIN_BASE') . '/GameItemListById');
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($ch, CURLOPT_HEADER, 0);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
        ...
        $stokJson = json_encode(['StockCode' => $paket->stockCode]);
        $ch = curl_init();
        $headers = ['Authorization: ' . getAuthName(), 'ApiName: ' . getApiName(), 'ApiKey: ' . getApiKey(), 'Content-Type: application/json'];
        curl_setopt($ch, CURLOPT_URL, env('EPIN_BASE') . '/CheckOrderProduct');
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($ch, CURLOPT_HEADER, 0);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
        ...
        $stok = DB::table('games_packages_codes')
                    ...
                    <div class="container">
                        ...
                        <form method="post" class="epin-form" action="{{ route('epin_satin_al') }}" autocomplete="off">
                            ...
                            <div class="row overflow-table">
                                            ...
                                            */ ?>
                                            ...
                                            <td>
                                                <div class="decrement">
                                                    ...
                                                    <input name="adet" maxlength="5" class="input-number" type="number"
                                                        id="adet" value="{{ $adet }}" min="1">
                                                    <span class="input-number-increment">+</span>
                                                </div>

                                            </td>
                                            <td><i>₺</i>{{ MF($satis_fiyati) }}</td>
                                ...
                                @if($epin->siparis==1)
                                    <?
                                    $soru=DB::table('epin_soru')->where('game_id',$epin->game)->first();
                                    ?>
                                <input type="hidden" name="siparis" value="316">

@if(!empty($soru->soru1) && strlen($soru->soru1)>2)
                                    <div class="row" style="display: flex; justify-content: center">
                                        <div class="col-6">
                                            <label for="soru1">{{$soru->soru1}}</label>
                                            <input id="soru1" type="text" name="cvp1" class="form-control" placeholder="{{$soru->soru1}}" >
                                        </div>
                                    </div>
@endif
@if(strlen($soru->soru2)>2)
                                    <div class="row" style="display: flex; justify-content: center">
                                        <div class="col-6">
                                            <label for="soru2">{{$soru->soru2}}</label>
                                            <input id="soru2" type="text" name="cvp2" class="form-control" placeholder="{{$soru->soru2}}" >
                                        </div>
                                    </div>
@endif
                               @endif
                                        ...
                                        <div class="card-body">
                                            ...
                                            @if (isset(Auth::user()->id))
                                                @if (Auth::user()->bakiye + Auth::user()->bakiye_cekilebilir >= $fiyat)
                                                    ...
                                                    <?php
                                                            /*@if($epin->epin != 0) <input type="hidden" name="stokKodu" value="{{$paket->stokKodu}}"> @else <input type="hidden" name="stokKodu" value="0"> @endif */
                                                    ?>
                                                    @if ($paket->stockCode != null)
                                                    ...
                                                    <button type="submit" id="buton" class="btn btn-primary mt-3">@lang('general.onayla')</button>
                                                    @include('front.plugins.siparisiniz-isleniyor')
                                                @else
                                                @php LogCall(Auth::user()->id, '2', "Kullanıcının satın alma için yeterli bakiyesi yok. Kullanıcının toplam bakiyesi: " . (Auth::user()->bakiye + Auth::user()->bakiye_cekilebilir) . ". Kullanıcının almak istediği ürün: " . $paket->title . ". Almak istenen miktar: " . $adet . " Ürün Fiyatı: " . $fiyat); @endphp
...
@if($epin->siparis==1)
    ...
    if($('#soru1').length>=1) {
        $('#soru1').keyup(function () {if ($('#soru1').val() == '') {$('#buton').attr('disabled', 'disabled');} else {$('#buton').removeAttr('disabled');}});
        $('#soru1').change(function () {if ($('#soru1').val() == '') {$('#buton').attr('disabled', 'disabled');} else {$('#buton').removeAttr('disabled');}});
    }

    if($('#soru2').length>=1) {
        $('#soru2').keyup(function () {if ($('#soru2').val() == '' || $('#soru1').val() == '') {$('#buton').attr('disabled', 'disabled');} else {$('#buton').removeAttr('disabled');}});
        $('#soru2').change(function () {if ($('#soru2').val() == '' || $('#soru1').val() == '') {$('#buton').attr('disabled', 'disabled');} else {$('#buton').removeAttr('disabled');}});
    }
@endif

        $('#adet').keyup(function(x) {
            fiyatHesapla($(this).val());
        });
                ...
                function init(el) {
                    ...
                    function increment() {
                        ...
                        @else
                            let bakiye = 0;
                        @endif
                        let urunFiyati = parseFloat({{ $satis_fiyati }});
                        if (!max || value <= max) {
