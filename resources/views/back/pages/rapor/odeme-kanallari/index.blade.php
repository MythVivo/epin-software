            ...
            <div class="card">
                <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8 col-sm-6 text-right">
                                ...
                                @else
                                    ...
                                    ?>
                                @endif
                                <form method="get" autocomplete="off" class="odeme-kanal-raporu">
                                    <div class="row">
                                        <div class="col-3">
                                            <div class="form-group">
                                                <label class="form-label" for="userinput1">İlk Tarih</label>
                                                <input type="date" id="userinput1" class="form-control" name="date" value="{{$date}}">
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                        <div class="form-group">
                                                <label class="form-label" for="userinput2">Son Tarih</label>
                                                <input type="date" id="userinput2" class="form-control" name="date2" value="{{$date2}}">
                                            </div>
                                        </div>
                                        <div class="col-4">
                                            <div class="form-group">
                                                <select name="channel[]" class="form-control" multiple style="height: 160px;">
                                                    <option value="0" @if(in_array("0", $channel))  selected @endif>Hepsi</option>
                                                    @foreach(DB::table('payment_channels')->get() as $u)
                                                        <option value="{{$u->id}}" @if(in_array($u->id, $channel)) selected @endif >{{$u->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-2">
                                            <button type="submit" class="btn btn-outline-success waves-effect waves-light">Sorgula</button>
                                        </div>
                                    </div>
                    ...
                    <div class="table-responsive">
                        {{view('back.pages.rapor.odeme-kanallari.table')}}
                    </div>
                </div>
            </div>
