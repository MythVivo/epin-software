        ...
        @foreach(\App\Models\Games::where('lang', getLang())->get() as $u)
            ...
            <div class="col-lg-3 mb-4" >
                <div class="card card-border client-card game">
                    ...
                    <img class="card-img-top img-fluid" src="{{asset(env('root').env('front').env('games').$image)}}"
                         alt="Card image cap">
                    <div class="card-body text-center">
                        <h2 class="client-name">
                            <span>{{categoryFind($u->category)}}</span>
                            -
                            <span>{{$u->title}}</span></h2>
                        <br>
                        <h4>@lang('admin.oyunBasliklar')</h4>
                        @foreach(\App\Models\GamesTitles::where('game', $u->id)->get() as $uu)
                            <h5 class="altBaslik">{{$uu->title}}</h5>
                        @endforeach
                        <div class="col-12">
                            <hr>
                        </div>
                        <div class="card-bottom">
                            <button onclick="location.href='{{route('oyun_detay', $u->link)}}'" type="button" class="btn btn-block btn-outline-secondary">@lang('admin.detay')</button>
                            ...
                            <div class="modal-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <label for="4">@lang('admin.oyunResmi')</label>
                                            <input data-default-file="{{asset(env('root').env('front').env('games').'/'.$u->image)}}" name="image" type="file" id="4" class="dropify"
                                                   accept="image/*">
                                        </div>
                                    </div>
                                </div>
