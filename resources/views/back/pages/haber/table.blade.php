<table lang="{{getLang()}}" id="datatable" class="table table-bordered nowrap"
    ...
    @foreach(\App\Models\News::where('lang', getLang())->whereNull('deleted_at')->get() as $u)
        <tr id="row-{{$u->id}}">
            <td>{{$u->title}}</td>
            <td>{{$u->text_short}}</td>
            <td class="text-center"><img style="max-width: 50%;" src="{{asset(env('root').env('front').env('news').$u->image)}}"></td>
            <td>
                <a href="{{$u->link}}"
            ...
            <td>
                ...
                <button data-toggle="modal" data-target=".duzenle" onclick="edit({{$u->id}}, 'news', event)" type="button"
