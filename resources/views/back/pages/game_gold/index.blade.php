            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        {{ view('back.pages.game_gold.table') }}
                    </div>
                </div>
            </div>
...
@section('js')
                ...
                "footerCallback": function(row, data, start, end, display) {
                    ...
                    while (e < 7) {
                        ...
                        sumCol4Filtered = display.map(el => data[el][e]).reduce((a, b) => intVal2(a) +
                            intVal2(b), 0);
                        $(api.column(e).footer()).html(sumCol4Filtered.toFixed(0) + " TL");
                        e++;
                    }
                },
                columnDefs: [{
    ...
    <script>
        function confirme(x, id) {
            var d = x == 2 ? 'İPTAL edilecek ' : 'Onaylanacak ';
            swal.fire({
                title: 'İşlem ' + d + 'Emin misiniz ?',
                text: x == 2 ? "İptal sebebi giriniz." : "Devam edilsin mi ?",
                icon: "info",
                type: 'error',
                ...
                if (result.value) {
                    ...
                    {
                        $.ajax({
                            url: 'https://oyuneks.com/panel/game-gold-yonetim-onayla/2/' + id,
                            method: "POST",
                            data: { notx : result.value,_token:$('meta[name="csrf-token"]').attr('content') },
                            dataType: "html",
                    ...
                    {
                        $.get('https://oyuneks.com/panel/game-gold-yonetim-onayla/' + x + '/' + id);
                    }
                    swal.fire(
                        '{{ __('admin.basarili') }}',
    ...
    <script>
        setInterval(function() {
            $.get("{{ route('getGameGoldCont') }}", function(data) {
                if (data == "1") {
                    swal.fire({
                        title: "Yeni Sipariş",
