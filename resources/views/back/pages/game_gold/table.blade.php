...
<table lang="{{ getLang() }}" id="datatable" class="font-12 table-sm table-bordered table-hover nowrap"
    ...
    <tbody>
        ...
        if ($status == 3) {
        	...
        	WHERE isnull(ggs.deleted_at) and date(ggs.created_at) BETWEEN ? and ? order by ggs.id desc",
                [$date1, $date2],
            );
        } else {
            $sorgu = DB::select(
        	...
        	inner join users u on u.id=ggs.user
        	inner join games_packages_trade gpt on gpt.id=ggs.paket
        	WHERE ggs.STATUS=? and isnull(ggs.deleted_at) and date(ggs.created_at) BETWEEN ? and ? order by ggs.id desc",
                [$status, $date1, $date2],
            );
        }
        /*
        ...
        @foreach ($sorgu as $u)
            ...
            <tr @if ($u->status == 0) @if ($u->tur == 'bizden-al') class="bg-orange text-white"
                @elseif($u->teslim_nick != null and $u->tur == 'bize-sat') class="bg-beanred text-white"
                    @else class="bg-info text-white" @endif
                @endif>
                ...
                <td <?if(strpos($u->title,'Rise On')!==false) {echo "class='bg-blue-gradient text-white'";}
                    ?>>{{ $u->title }}</td>
                <td>{{ $u->note }}</td>
                <td>{{ $u->adet }}</td>
                <td>{{ $tur }}</td>
                    ...
                    @if ($u->status == 0)
                        ...
                        @elseif($u->teslim_nick != null and $u->tur == 'bize-sat')
                            <span class="text-blue"
                                title="{{ $u->teslim_nick }}"><? echo substr($u->teslim_nick,0,30);?>..</span>
                        @endif
                    @elseif($u->status == 1)
                        Satış Tamamlandı
                    @elseif($u->status == 2)
                        @if($u->notx)
                        Satış İptal Edildi : {{$u->notx}}
                        @else
                        Satış İptal Edildi
                        @endif
                    @else
                ...
                <td align="center">
                    @if ($u->status == 0)
                        @if ($u->tur == 'bize-sat')
                            <i data-toggle="modal" data-target="#nickGir{{ $u->id }}"
                                class="btn fas fa-pen"></i>
                            <div class="modal fade" id="nickGir{{ $u->id }}" tabindex="-1"
                                        ...
                                        <form method="post"
                                            ...
                                            <div class="modal-body">
                                                <div class="form-group">
                                                    ...
                                                    <input placeholder="Teslim edecek nick" type="text"
                                                        name="teslim_nick" class="form-control" required>
                                                </div>
                                            </div>
                            ...
                            @endif
                            <? // location.href='{{ route('game_gold_yonetim_onayla', [1, $u->id]) }}' ?>
                        @else
                            <i onclick="confirme(1,<?= $u->id ?>)" class="btn fas fa-check"></i>
                        @endif
                    @endif
                    ...
                    @if (userRoleIsAdmin(Auth::user()->id))
                        <i onclick="deleteContent('game_gold_satis', {{ $u->id }})"
                            class=" btn far fa-trash-alt"></i>
                    @endif
                </td>
            </tr>
        @endforeach
