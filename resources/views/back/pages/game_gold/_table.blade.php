	...
	*/
    ...
    @foreach($sorgu as $u)
        ...
        <tr @if($u->status == 0) 
                ...
                @elseif($u->teslim_nick != NULL and $u->tur == "bize-sat") class="bg-orange text-white"
            ...
            <td>
                ...
                <a class="text-white" href="{{route('uye_detay', [$u->email])}}" target="_blank">
                    {{$u->name}}
                </a>
            </td>
            <td>{{$u->title}}</td>
            ...
            <td align="center">
                @if($u->status == 0)
                    @if($u->tur == "bize-sat")
                        ...
                        @if($u->teslim_nick != NULL)
                                <i onclick="location.href='{{route('game_gold_yonetim_onayla', [1, $u->id])}}'" class="btn fas fa-check"></i>
                        @endif
                    @else
                            <i onclick="location.href='{{route('game_gold_yonetim_onayla', [1, $u->id])}}'" class="btn fas fa-check"></i>                       
                    @endif
                @endif
                @if($u->status == 0)
                        <i onclick="location.href='{{route('game_gold_yonetim_onayla', [2, $u->id])}}'" class=" btn fas fa-times"></i>
                @endif
