<?
    if(@$_GET['sat_ipt']!=''){
        $p = (object) $_GET;
        $tar=date('YmdHis');
        DB::select("update pazar_yeri_ilanlar_buy set status=1 where id='$p->sat_ipt'");
        $tel=DB::select("select u.telefon from pazar_yeri_ilanlar_buy py join users u on u.id=py.user where py.id='$p->sat_ipt'");
        DB::select("update pazar_yeri_ilan_satis_buy set deleted_at='$tar' where ilan='$p->sat_ipt'");
        sendSms($tel[0]->telefon,"Satıcı teslimata gelmedigi icin ilanınıza yapılan satım talebi iptal edilmistir.");
        die();
    }
?>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        {{view('back.pages.ilanlar-buy.table')}}
                    </div>
                </div>
            </div>
...
@section('js')
    ...
    <script>
        ...
        $('.iptal').click(function(){
            if(confirm("Bu işlem sonucunda; \nSatıcının satma talebi iptal edilecek,\nAlıcı işlemin iptali konusunda bilgilendirilecek \nMevcut alım ilanı yayında kalacak \n\nİşleme devam edilsin mi ?")) {
                $.get('', {sat_ipt: $(this).attr('id') }, function (x) {location.reload();});
            } else {
