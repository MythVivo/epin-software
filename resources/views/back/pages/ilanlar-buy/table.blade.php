<table lang="{{getLang()}}" id="datatable" class="font-12 nowrap table-hover table-sm table-bordered"
    ...
    @foreach(DB::table('pazar_yeri_ilanlar_buy')->whereNull('deleted_at')->get() as $u)
        <tr style="background-color:@if($u->status==0) darkgreen; @endif @if($u->status==4) blueviolet; @endif @if($u->status==5) darkred; @endif ">
            <td>{{$u->id}}</td>
                                                    ...
                                                    @if($u->status == 4 or $u->status == 5)
                                                        <div class="col">
                                                            ...
                                                            @if($u->status == 5)
                                                                ...
                                                                @php
                                                                    $blokeTutar =     $u->price * 0.1;

                                                                @endphp
                                                                    <h4>Üye Bakiyesi : ₺{{$uye->bakiye+$uye->bakiye_cekilebilir}}</h4>
                                                                    @if($uye->bakiye +$uye->bakiye_cekilebilir< ($u->price - $blokeTutar))
                                                                        <h4 class="text-danger">Yüklemesi Gereken Bakiye
                                                                            :
                                                                            ₺{{$uye->bakiye +$uye->bakiye_cekilebilir - ($u->price - $blokeTutar)}}</h4>
                                                                    @endif
                                                                    <h4 class="text-danger">Henüz Alıcıdan para tahsil
                                                                        edilmedi, öncelikle alıcıdan parayı tahsil
                                                                        ediniz.</h4>
                                                                    <button onclick="location.href='{{route('ilanlar_yonetim_buy_onay', [$u->id, '66'])}}'" class="btn btn-outline-success w-100"
{{-------------------------------------------------------------------------------------------------------                                                                            --}}
                                                                            @if($uye->bakiye+$uye->bakiye_cekilebilir < ($u->price - $blokeTutar)) disabled style="cursor:not-allowed;" @endif>Alıcıdan Parayı Çek </button>
                                                                @else
                                                                        <h4 class="text-blue">Alıcıdan para tahsil edildi, henüz satıcıya parası gönderilmedi.</h4>
                                                                    <button onclick="location.href='{{route('ilanlar_yonetim_buy_onay', [$u->id, '6'])}}'" class="btn btn-outline-success w-100 mb-3">Satış Başarılı Olarak İşaretle Ve Parayı Satıcıya Gönder                                                                    </button>
                                                                @endif
                                                                    @if($u->aliciPara == 1)
                                                                        <button onclick="location.href='{{route('ilanlar_yonetim_buy_onay', [$u->id, '98'])}}'" class="btn btn-outline-danger w-100"> !!..Site ALICI ya itemi teslim edemiyor. ALICI ve SATICI nın parasını İADE et..!!</button>
                                                                    @else
                                                                        <button onclick="location.href='{{route('ilanlar_yonetim_buy_onay', [$u->id, '99'])}}'" class="btn btn-outline-orange w-100">ALICI nın bakiyesi yetersiz, yükleme yapmadı iptal (OLUMSUZ)</button>
                                                                    @endif
                                                            @endif
                                                        </div>
                                                    @endif
                                                    @if($u->status < 3)
                                                        <div class="col">
                                                            <button onclick="location.href='{{route('ilanlar_yonetim_buy_onay', [$u->id, '3'])}}'"
                                                                    class="btn btn-outline-primary w-100">İlanı Pasif Hale Getir
                                                            </button>
                                                        </div>
                                                    @endif
                                                </div>
                                                ...
                                                @if(DB::table('pazar_yeri_ilan_satis_buy')->where('ilan', $u->id)->count() > 0)
                                                 ...
                                                 <button id="{{$u->id}}" class="btn btn-outline-danger col-6 m-3 iptal">İşlemi İPTAL Et</button>
