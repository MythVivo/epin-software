            ...
            <div class="card">
                <div class="card-body">
                    <h4 class="mt-0 header-title mb-3">
                        <div class="row">
                            <div class="text-left bg-soft-danger" style="display: flex;align-items: center; white-space: nowrap">
                                Yorum Periyodu <input type="number" value="{{DB::table('settings')->first()->yorum_sure}}" id="sure" class="form-control" style="width: 60px"> dk.
                            </div>
                        </div>
                    </h4>
                    <div class="table-responsive">
                        {{view('back.pages.yorumlar.table')}}
                    </div>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#sure').change(function (){
                $.post('/ykp.php', {yorum_sure:$(this).val()}, function (x) {
                    swal.fire({icon:'success',title:'Süre kayıt edildi',showConfirmButton: false,timer:1500});
                });
            })
