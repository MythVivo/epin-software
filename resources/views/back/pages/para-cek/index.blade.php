<?php
if (@$_GET['pgonder']) {
    $q = explode('_', $_GET['pgonder']);
    $user = $q[0];
    $pid = $q[1];
    $tut = $q[2];
    $hesap = $q[3];
    $pendingReq = DB::table('integrations_papara')->where('para_cek_id', $pid)->first();
    if ($pendingReq) {
        if ($pendingReq->state == 1) {
            echo 'Mukerrer';
            die();
        }
    } else {
        DB::table('integrations_papara')->insert([
            'para_cek_id' => $pid,
            'request' => json_encode(['user' => $user, 'tutar' => $tut, 'hesap' => $hesap]),
            'state' => -1,
        ]);
        $pendingReq = DB::table('integrations_papara')->where('para_cek_id', $pid)->first();
    }

    $responses = paparaCurl::send($hesap, $tut);
    ...
    DB::table('integrations_papara')->where('para_cek_id', $pid)->update(['state' => $durum, 'response' => $responses]);

    if ($durum == 1) {
        DB::table('para_cek')->where('id', $pid)->update(['status' => 1, 'updated_at' => date('YmdHis')]);
    }
    echo $durum;
    echo $hata;
    die();
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        {{ view('back.pages.para-cek.table') }}
                    </div>
                </div>
            </div>
...
@section('js')
    ...
    <script>
        $('.pgon').click(function(x) {
            var id = $(this).attr('id');
            var trr = $(this);
            if (id) {
                if (confirm("Kullanıcının PAPARA hesabına gönderim yapılacak. Devam edilsin mi ?")) {
                    $.get('', {
                        pgonder: id
                    }, function(x) {
                        if (x == 1) {
                            alert("Gönderim başarılı");
                            trr.closest('tr').hide('slow');
                        } else {
                            alert("Hata oluştu, gönderim yapılamadı.. " + x)
                        }
                    });
                }
            }
