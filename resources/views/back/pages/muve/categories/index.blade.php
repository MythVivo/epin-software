...
@section('body')
    <?php
    $muveGamesCategories = muveGetCategories();
    ?>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="modal fade ekle" tabindex="-1" role="dialog" aria-hidden="true">
                        <div class="modal-dialog modal-xl">
                            <div class="modal-content">
                                ...
                                <form id="yeniEkle" method="post" action="{{route('muve_kategori_add')}}"
                                    ...
                                    <div class="modal-body">
                                        <div class="row">
                                            <div class="col-sm-6">
                                                <div class="form-group">
                                                    ...
                                                    <select name="muveId" class="select2 form-group">
                                                        <option selected disabled>Bir Kategori ID Seçin</option>
                                                        @foreach($muveGamesCategories as $u)
                                                            @if(DB::table('muve_games_categories')->where('muve_id', $u->id)->whereNull('deleted_at')->count() < 1)
                                                                <option value="{{$u->id}}"
                                                                        data-parent-id="{{$u->parent_id}}"
                                                                        data-names="{{json_encode($u->names)}}">{{$u->id}}</option>
                                                            @endif
                                                        @endforeach
                                                    </select>
                                                </div>
                                            ...
                                            <div class="col-sm-6">
                                                <div class="form-group">
                                                    ...
                                                    <select name="parent_id" class="form-control form-group">
                                                        ...
                                                        <option value="null">Genel Kategori</option>
                                                        @foreach($muveGamesCategories as $u)
                                                            @if(DB::table('muve_games_categories')->where('parent_id', null)->where('id', $u->id)->whereNull('deleted_at')->count() > 0)
                                                                <option value="{{$u->muve_id}}">{{$u->title}}</option>
                                                            @else
                                                                @if($u->parent_id == null)
                                            ...
                                            <div class="col-sm-12">
                                                <table class="table mb-5" id="isimlerTable" style="display: none;">
                                                    <thead>
                                                    <tr>
                                                        <th>Ülke</th>
                                                        <th>İsmi</th>
                                                    ...
                                                    <tbody id="isimlerTbody" class="text-left">
                                                    </tbody>
                                                </table>
                                            </div>
                                            <div class="col-md-12">
                    ...
                    <div class="table-responsive">
                        {{view('back.pages.muve.categories.table')}}
                    </div>
                </div>
            </div>
        </div> <!-- end col -->
    ...
    <script type="text/javascript">
        $(function () {
            $(document).on("change", "select[name='muveId']", function (e) {
                let parentId = e.target[e.target.selectedIndex].attributes["data-parent-id"].value;
                let names = JSON.parse(e.target[e.target.selectedIndex].attributes["data-names"].value);
                if(parentId) {
                    $("select[name='parent_id']").val(parentId);
                } else {
                    $("select[name='parent_id']").val('null');
                }
                $("#isimlerTable").show();
                var table = document.getElementById("isimlerTbody");
                table.innerHTML = '';
                for (let i in names) {
                    ...
                    x.setAttribute("id", "title" + i);
                    x.setAttribute("autocomplete", "off");
                    x.setAttribute("required", "");
                    x.value = names[i].value;
                    var row = table.insertRow(0);
                    var cell1 = row.insertCell(0);
                    var cell2 = row.insertCell(1);
                    var cell3 = row.insertCell(2);
                    cell1.innerHTML = names[i].locale;
                    cell2.innerHTML = names[i].value;
                    cell3.innerHTML = x.outerHTML;
                }
            });
        });
