...
{{--                    <button class="btn btn-sm ekle btn-success">Yeni Kur</button>--}}
                    <div class="table-responsive">
                        ...
                        <table lang="{{getLang()}}" id="datatable" class="font-12 table-sm table-bordered table-hover nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            <thead><tr style="text-align: center"><th>Id</th><th>Kur</th><th>Oran</th><th>Dzn</th></tr></thead>
                            <tbody>


{{--                            @php--}}
{{--                                $cg=DB::table('cari_kurlar')->whereNull("deleted_at")->get();--}}
{{--                            @endphp--}}
{{--                            @foreach($cg as $u )--}}
{{--                                <tr>--}}
...
{{--                                    <td style="text-align: center"><i title="Sil" id="{{$u->id}}" class="btn far fa-trash-alt sil"></i><i title="Düzenle" id="{{$u->id}}" class="btn far fa-edit dzn"></i></td>--}}
{{--                                </tr>--}}
{{--                            @endforeach--}}
        ...
        $('.ekle').click(function(){
            ...
            swal.fire({
                html: "<br>Kur Tanımlama<br>" +
                    "<div class='form-floating'><input id='ad' placeholder='Kur Cinsi' class='form-control border' type='text'><label>Kur Cinsi</label></div>" +
                ...
                if (result.value) {
                    $.post('{{route('cari_api')}}', {ad:$('#ad').val(), oran:$('#oran').val(), kur_ekle:316, _token:$('meta[name="csrf-token"]').attr('content')}, function (x) {location.reload();});
                }
            });
            ...
            $.post('cari_api', {id:id, kur_oku:316, _token:$('meta[name="csrf-token"]').attr('content')}, function (x) { let y=JSON.parse(x);
                swal.fire({
                    html: "<br>Kur Düzenleme<br>" +
                        "<div class='form-floating'><input id='ad' value='"+y[0].kur+"' placeholder='Kur Cinsi' class='form-control border' type='text'> <label>Kur Cinsi</label></div>" +
                    ...
                    if (result.value) {
                        $.post('{{route('cari_api')}}', {id:id, oran:$('#oran').val(), kur_dzn:316, ad:$('#ad').val(), _token:$('meta[name="csrf-token"]').attr('content')}, function (x) {location.reload();});
                    }
                });
            });
        });
